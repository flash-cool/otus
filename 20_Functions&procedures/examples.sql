...
-- два способа созданий SQL-функций
...
RETURNS integer
LANGUAGE sql
RETURN (SELECT count(1) FROM the_tab);
...
SELECT oid, proname, prosrc, prosqlbody
FROM pg_proc
WHERE proname LIKE 'fnc_cnt%';

CREATE TABLE new_tab
AS
SELECT nmb FROM the_tab
UNION ALL
SELECT nmb FROM the_tab;
-------------------------------------------------------------

ALTER TABLE the_tab RENAME TO old_tab;
ALTER TABLE new_tab RENAME TO the_tab;

SELECT fnc_cnt_old();
SELECT fnc_cnt_new();
-------------------------------------------------------------

-- несколько операторов в "ANSI-style" функции
CREATE FUNCTION fnc_multistatement()
RETURNS integer
BEGIN ATOMIC
    SELECT nmb FROM the_tab ORDER BY nmb ASC;
    SELECT nmb FROM old_tab ORDER BY nmb DESC;
END;    -- в DBeaver не выполянется :-(

SELECT fnc_multistatement();
-------------------------------------------------------------
...
UPDATE tab_over
SET the_value = ROW(1, 1.1, 'No string')
WHERE id = 2;
...
DECLARE
    ...
    second_val  ALIAS FOR $2;
BEGIN
    RETURN add.first_val + add.second_val;  -- недопустимо
END
$BODY$
LANGUAGE plpgsql;
...
DECLARE
    ...
    second_val  ALIAS FOR $2;
BEGIN
    RETURN main_block.first_val + main_block.second_val;  -- допустимо
END
$BODY$
LANGUAGE plpgsql;
...
CREATE TABLE sales
(
	sales		integer GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
BEGIN
    ...
    LOOP
                /* format см. в 9.4.1.*/
        query = format('INSERT INTO tmp_tables (table_name, column_name, column_str) SELECT %s, %s, %s FROM %I', quote_literal(v_table_name), quote_literal(v_column_name), v_column_name, v_table_name);
...
CREATE OR REPLACE FUNCTION fnc(p_tab_name text, p_col_name text)
RETURNS record
AS
$$
DECLARE
...
BEGIN
    -- v_query = 'SELECT * FROM ' || p_tab_name || ' WHERE ' || p_col_name || ' = 1;';
    v_query = format ('SELECT * FROM %I WHERE %I = 1;', p_tab_name, p_col_name);
...
SELECT fnc('t', 'id = 1; DROP TABLE t; SELECT * FROM pg_class WHERE oid');
SELECT fnc('t', 'id');

ROLLBACK;
START TRANSACTION;
SELECT * FROM t WHERE id = ; DROP TABLE t; SELECT * FROM pg_class WHERE oid = 1;
...
CREATE TABLE test (ii integer);


CREATE PROCEDURE proc_tr()
AS
$$
BEGIN
